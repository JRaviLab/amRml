...
dplyr::summarise(mean_rank_nonshuffle = mean(mean_rank[shuffled == FALSE], na.rm = TRUE),    
...
dplyr::filter(!is.na(cluster), shuffled == FALSE) |>
dplyr::group_by(species, drug_label,drug_or_class, cluster) |> 
dplyr::summarize(median_contribution = median(contribution), 
          median_rank = median(mean_rank), .groups = "drop") |> 
dplyr::left_join(protein_scale_summary |> 
