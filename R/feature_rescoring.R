computeFeatureImprovement <- function(
  ...
  features_rescored <- arrow::read_parquet(normalizePath(all_top_features_parquet)) |>
    ...
    dplyr::select(
      ...
      feature_type, feature_subtype, seed, Variable,
      Importance, Sign
    ) |>
    dplyr::mutate(
    ...
    dplyr::group_by(drug_label, drug_or_class, feature_type, feature_subtype, seed) |>
    dplyr::mutate(
    ...
    dplyr::ungroup() |>
    dplyr::group_by(drug_label, drug_or_class, feature_type, feature_subtype, Variable) |>
    ...
    dplyr::mutate(
      median_cluster = stats::median(median_drug_or_class, na.rm = TRUE),
      count_features_for_cluster = dplyr::n_distinct(Variable)
    ) |>
    dplyr::ungroup()
