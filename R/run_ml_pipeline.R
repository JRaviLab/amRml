...
runMLPipeline <- function(ml_input_tibble, model = "LR", split = c(0.6, 0.2), 
  ...
  .checkArgShuffleLabels(shuffle_labels); .checkArgVerbose(verbose)
  .checkArgReturnTuneRes(return_tune_res); .checkArgReturnFit(return_fit)
  .checkArgReturnPred(return_pred)
  
  # Set `n_fold` to `NA` if not using cross-validation.
  if(split[2] != 0) {n_fold <- NA}
  ...
  if(getTargetVarName(ml_input_tibble) == "resistant_classes") {
    multi_class <- TRUE} else {multi_class <- FALSE}
  
  if(model != "LR" & multi_class) {
    stop(paste("Multi-class classification can be performed only with", 
      "logistic regression in our current implementation."))}
  
  if(verbose) {
    message(
      paste("`ml_input_tibble` has", num_obs_ml_input_tibble, "observations of", 
  ...
  if(!multi_class) {
    ...
    if(verbose) {print(rs_props_original_ml_input_tibble)}
    ...
    res_prop_original_ml_input_tibble <- rs_props_original_ml_input_tibble |> 
      ...
      dplyr::select(prop) |> as.numeric() |> round(2)
  ...
  if(shuffle_labels) {ml_input_tibble <- shuffleLabels(ml_input_tibble)}
  ...
  if(tibble::is_tibble(test_data)) {
    ...
    if(multi_class) {
      stop(paste("Multi-class classification cannot be performed with", 
        "external test data with the current implementation."))}
    
    num_obs_test_data <- nrow(test_data)
    
    if(verbose) {message(paste("`test_data` has", num_obs_test_data, 
    ...
    if(verbose) {print(rs_props_test_data)}
    ...
    # external `test_data`
    if(is.na(res_prop_test_data)) {res_prop_test_data <- 0}
  ...
  if(external_test_data & split[2] != 0) {
    ...
    val_indices <- (1:nrow(original_ml_input_tibble))[!(1:nrow(
      original_ml_input_tibble) %in% train_indices)]
  ...
  if(model == "LR") {
    ...
    grid <- buildTuningGrid(model = model, penalty_vec = penalty_vec, 
      mix_vec = mix_vec)
  }
  ...
  tune_res <- tuneGrid(wflow = wflow, data_split = data_split, grid = grid, 
    n_fold = n_fold)
  ...
  top_feat_tibble <- extractTopFeats(fit, n_top_feats = n_top_feats, 
  ...
  if(!multi_class & !is.na(n_top_feats) & nrow(top_feat_tibble) < n_top_feats) {
    ...
    top_feat_tibble <- tibble::tibble(Variable = top_feats, 
      Importance = importances, Sign = signs)
  }
  
  if(!multi_class) {n_feats_returned <- nrow(top_feat_tibble)}
  
  if(verbose) {message("Finished pipeline.")}
  ...
  # importance).
  performance_tibble <- tibble::tibble(num_obs = num_obs_ml_input_tibble, 
  ...
  if(!multi_class) {
    performance_tibble <- performance_tibble |> 
      tibble::add_column(res_prop = res_prop_original_ml_input_tibble, 
        .after = "num_obs") |> 
      tibble::add_column(n_feats_returned, .before = "n_fold") |> 
      tibble::add_column(upper_prop_vi_top_feats = prop_vi_top_feats[2], 
        .after = "val_prop") |> 
  ...
  all_results <- list(performance_tibble = performance_tibble, 
    top_feat_tibble = top_feat_tibble)
  
  if(return_tune_res) {all_results[["tune_res"]] <- tune_res}
  
  if(return_fit) {all_results[["fit"]] <- fit}
