...
loadMLInputTibble <- function(parquet_path) {
  ...
  if(!xor("genome_drug.resistant_phenotype" %in% colnames(long_tibble), 
    "resistant_classes" %in% colnames(long_tibble))) {
    stop(paste("The tibble loaded from `parquet_path` must have a target", 
  ...
  if(!all(c("genome_id", "feature_id", "value") %in% colnames(long_tibble))) {
    stop(paste("The data is missing one or more of the following required", 
  ...
  ml_input_tibble <- long_tibble |> 
    ...
    tidyr::pivot_wider(id_cols = dplyr::all_of(
...
shuffleLabels <- function(ml_input_tibble, seed = 123) {
  .checkArgTibble(ml_input_tibble, ml = TRUE); .checkArgSeed(seed)
...
calculateMinSamples <- function(n_fold, split, res_prop, 
  smallest_n_obs_rs = 1) {
  .checkArgSplit(split); .checkArgResProp(res_prop)
